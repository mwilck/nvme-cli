...
struct fabrics_config fabrics_cfg = { .ctrl_loss_tmo = -1 };
...
int do_discover(char *argstr, bool connect)
{
	...
	if (fabrics_cfg.device) {
		...
		/*
		 * if the fabrics_cfg.device passed in matches the connect args
		 *    fabrics_cfg.device is left as-is
		 * else if there exists a controller that matches the
		 *         connect args
		 *    fabrics_cfg.device is the matching ctrl name
		 * else if no ctrl matches the connect args
		 *    fabrics_cfg.device is set to null. This will attempt to
		 *    create a new ctrl.
		 * endif
		 */
		if (!ctrl_matches_connectargs(fabrics_cfg.device, cargs))
		...
		const struct argconfig_commandline_options *opts, bool connect)
{
	...
	if (f == NULL) {
		msg(LOG_ERR, "No discover params given and no %s conf\n",
			PATH_NVMF_DISC);
		return -EINVAL;
	}
	...
	while (fgets(line, sizeof(line), f) != NULL) {
		...
		if (!argv) {
			msg(LOG_ERR, "failed to allocate argv vector\n");
			free(args);
			ret = -ENOMEM;
			goto out;
		}
